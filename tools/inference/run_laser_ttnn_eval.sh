...
"$PYTHON_BIN" "$ROOT_DIR/tools/inference/ttnn_torch_compare.py" \
  --config "$CONFIG_PATH" \
  ...
  --score-thresh 0.3 \
  --torch-device cpu \
  --output-dir "$OUTPUT_DIR/laser_ttnn_${EVAL_SIZE}_${STAMP}"

echo "Saved results to $OUTPUT_DIR/laser_ttnn_${EVAL_SIZE}_${STAMP}"
